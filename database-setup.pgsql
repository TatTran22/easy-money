...
create or replace function public.on_delete_user()
  ...
  begin
    ...
    delete from public.transactions where creator_id = old.id;
...
CREATE TABLE transaction_categories (
  ...
  updated_at timestamp WITH time zone DEFAULT timezone('ict'::text,now()) NOT NULL,
	...
	UNIQUE (name),
	CONSTRAINT name CHECK (char_length(name) >= 3),
	CONSTRAINT category_type_check CHECK (type IN ('EXPENSE', 'INCOME', 'TRANSFER')),
...
CREATE TABLE transactions (
  ...
  CONSTRAINT currency_check CHECK (currency IN ('USD', 'EUR', 'VND', 'JPY', 'CNY', 'KRW', 'GBP')),
  CONSTRAINT transaction_type_check CHECK (transaction_type IN ('EXPENSE', 'INCOME', 'TRANSFER')),
  CONSTRAINT transaction_status_check CHECK (transaction_status IN ('PENDING', 'APPROVED', 'REJECTED')),
  CONSTRAINT privacy_check CHECK (privacy IN ('PRIVATE', 'FRIENDS', 'PUBLIC')),
  CONSTRAINT creator_id_fkey FOREIGN KEY (creator_id) REFERENCES auth.users (id) ON DELETE CASCADE,
...
create policy "Anyone can upload an category icon."
	on storage.objects for insert
	with check ( bucket_id = 'transaction_categories' );

create policy "Anyone can update an category icon."
	on storage.objects for update
	with check ( bucket_id = 'transaction_categories' );
...
create policy "Anyone can upload an budget icon."
  on storage.objects for insert
  with check ( bucket_id = 'budgets' );

create policy "Anyone can update an budget icon."
  on storage.objects for update
  with check ( bucket_id = 'budgets' );
...
create policy "Anyone can upload an transaction image."
  on storage.objects for insert
  with check ( bucket_id = 'transactions' );

create policy "Anyone can update an transaction image."
  on storage.objects for update
  with check ( bucket_id = 'transactions' );


-- DUMMY DATA
insert into budgets (id, creator_id, type, name, icon, created_at, updated_at)
values (uuid_generate_v4(), uuid_generate_v4(), 'INCOME', 'Salary', 'https://storage.googleapis.com/budget-app-storage/budgets/salary.png', now(), now());


insert into transaction_categories (id, name, creator_id, parent_id, created_at, updated_at)
values (1, 'Food', '00000000-0000-0000-0000-000000000000', null, timezone('ict', now()), timezone('ict', now()));

insert into transaction_categories (id, name, creator_id, parent_id, created_at, updated_at)
values (2, 'Transport', '00000000-0000-0000-0000-000000000000', null, timezone('ict', now()), timezone('ict', now()));

insert into transaction_categories (id, name, creator_id, parent_id, created_at, updated_at)
values (3, 'Shopping', '00000000-0000-0000-0000-000000000000', null, timezone('ict', now()), timezone('ict', now()));

insert into transaction_categories (id, name, creator_id, parent_id, created_at, updated_at)
values (4, 'Entertainment', '00000000-0000-0000-0000-000000000000', null, timezone('ict', now()), timezone('ict', now()));

insert into transaction_categories (id, name, creator_id, parent_id, created_at, updated_at)
values (5, 'Education', '00000000-0000-0000-0000-000000000000', null, timezone('ict', now()), timezone('ict', now()));

insert into transactions (id, amount, currency, transaction_type, transaction_status, privacy, creator_id, budget_id, transaction_category_id, created_at, updated_at)
values (1, 100, 'USD', 'EXPENSE', 'APPROVED', 'PRIVATE', '00000000-0000-0000-0000-000000000000', 1, 1, timezone('ict', now()), timezone('ict', now()));

insert into transactions (id, amount, currency, transaction_type, transaction_status, privacy, creator_id, budget_id, transaction_category_id, created_at, updated_at)
values (2, 200, 'USD', 'INCOME', 'APPROVED', 'PRIVATE', '00000000-0000-0000-0000-000000000000', 1, 2, timezone('ict', now()), timezone('ict', now()));

insert into transactions (id, amount, currency, transaction_type, transaction_status, privacy, creator_id, budget_id, transaction_category_id, created_at, updated_at)
values (3, 300, 'USD', 'EXPENSE', 'APPROVED', 'PRIVATE', '00000000-0000-0000-0000-000000000000', 1, 3, timezone('ict', now()), timezone('ict', now()));
